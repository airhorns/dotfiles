function _exec_with
  ...
  set -l variables_before (mktemp -t fish)
  set -l variables_after (mktemp -t fish)
  set -l functions_before (mktemp -t fish)
  set -l functions_after (mktemp -t fish)
  set -l aliases_before (mktemp -t fish)
  set -l aliases_after (mktemp -t fish)
  ...
  set -l var_count (count $argv)
  if test $var_count -gt 2
    /usr/bin/env $shell -c "$before; $source $file $argv[3..$var_count]; $after"
  else
    /usr/bin/env $shell -c "$before; $source $file; $after"
  end
